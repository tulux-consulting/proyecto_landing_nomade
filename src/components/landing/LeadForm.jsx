...
function LeadForm({ d }) {
    ...
    switch (step) {
      case 0: return (
        <div className="form-grid">
          ...
          <Field id="apellido" label="Apellido" error={errors.apellido}>
            <input value={vals.apellido} onChange={set("apellido")} placeholder="Tu apellido" autoComplete="family-name" />
          </Field>
          <Field id="email" label="Email" error={errors.email}>
            <input type="email" value={vals.email} onChange={set("email")} placeholder="nombre@correo.com" autoComplete="email" inputMode="email" />
          </Field>
          <Field id="telefono" label="Teléfono" error={errors.telefono}>
            <input type="tel" value={vals.telefono} onChange={set("telefono")} placeholder="+54 9 ..." autoComplete="tel" inputMode="tel" />
          </Field>
          <ChipField id="relacion" label="Tu relación con el terreno" full>
      ...
      case 1: return (
        <div className="form-grid">
          ...
          <Field id="localidad" label="Localidad o paraje" error={errors.localidad}>
            <input value={vals.localidad} onChange={set("localidad")} placeholder="Ciudad, pueblo o paraje" />
          </Field>
          <Field id="coords" label="Link de mapa o coordenadas" optional>
            <input value={vals.coords} onChange={set("coords")} placeholder="Google Maps o lat, long" />
          </Field>
          <Field id="distancia" label="Distancia a la ciudad más cercana" optional>
            <input value={vals.distancia} onChange={set("distancia")} placeholder="Ej.: 40 km · 30 min" />
          </Field>
        </div>);
      ...
      case 2: return (
        <React.Fragment>
          ...
          <ChipField id="topografia" label="Topografía" full>
        ...
        <React.Fragment>
          ...
          <div className="form-grid">
            <Field id="vistas" label="Vistas predominantes" optional>
              <input value={vals.vistas} onChange={set("vistas")} placeholder="Ej.: cordillera, lago, valle" />
            </Field>
            <Field id="entorno" label="Vegetación" optional>
              <input value={vals.entorno} onChange={set("entorno")} placeholder="Ej.: bosque nativo, monte, pastizal" />
            </Field>
          </div>
          <ChipField id="acceso" label="Tipo de acceso" full>
            <Chips options={d.accesos} value={vals.acceso} onChange={setVal("acceso")} />
          </ChipField>
          <ChipField id="estacionalidad" label="Disponibilidad de acceso" full>
          ...
          <Field id="aeropuerto" label="Aeropuerto o ciudad de referencia" optional full>
            <input value={vals.aeropuerto} onChange={set("aeropuerto")} placeholder="Ej.: Aeropuerto de Bariloche, 50 km" />
          </Field>
        </React.Fragment>);
      ...
      case 5: return (
        <React.Fragment>
          ...
          <ChipField id="usoSuelo" label="Uso de suelo" full>
          ...
          <Field id="legalNotas" label="Restricciones ambientales u observaciones legales" optional full>
            <input value={vals.legalNotas} onChange={set("legalNotas")} placeholder="Áreas protegidas, gravámenes, etc." />
          </Field>
        </React.Fragment>);
      ...
      case 6: return (
        <React.Fragment>
          ...
          <ChipField id="demanda" label="Demanda turística de la zona" full>
      ...
      case 7: return (
        <React.Fragment>
          ...
          <ChipField id="inversion" label="Disponibilidad de inversión" full>
            <Chips options={d.inversion} value={vals.inversion} onChange={setVal("inversion")} />
          </ChipField>
          <ChipField id="horizonte" label="Horizonte temporal" full>
      ...
      case 8: return (
        <React.Fragment>
          ...
          <div className="field full">
            ...
            <button type="button" className="uploader" onClick={() => fileRef.current && fileRef.current.click()} aria-labelledby="fotos-label">
  ...
  return (
    <section className="section sand" id="formulario" ref={formRef} aria-labelledby="form-title">
